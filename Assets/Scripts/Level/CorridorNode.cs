...
namespace Musashi.Level
{
    public class CorridorNode :Node
    {
        ...
        {
            Vector2 middlePointStructure1Temp = StructureHelper.CalculateMiddlePoint(structure1.BottomLeftAreaCorner, structure1.TopLeftAreaCorner);
            Vector2 middlePointSturcture2Temp = StructureHelper.CalculateMiddlePoint(structure2.BottomLeftAreaCorner, structure2.TopLeftAreaCorner);

            float angle = CalculateAngle(middlePointStructure1Temp, middlePointSturcture2Temp);

            if (angle < 45 && angle >= 0 || angle > -45 && 45 < 0) return RelativePosition.Right;
            if (angle > 45 && angle < 135) return RelativePosition.Up;
            if (angle > -135 && angle < -45) return RelativePosition.Down;
            return RelativePosition.Left;
        }

        private float CalculateAngle(Vector2 middlePointStructure1Temp, Vector2 middlePointSturcture2Temp)
        {
            return Mathf.Atan2(middlePointSturcture2Temp.y - middlePointStructure1Temp.y,
                middlePointSturcture2Temp.x - middlePointStructure1Temp.x) * Mathf.Rad2Deg;
        }
    }
}